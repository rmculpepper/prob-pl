    ...
    (define/public (summary-apply a)
      ...
      (match a
        [(node:apply fun args inner result)
         ...
         (cond [(exn? result)
                (insert (exn-message result) #:style error-sd)]
               [else (insert/abbrev (value->sexpr result) table #:limit 40)])
         (insert "\n")]
        [_ (void)]))
    ...
    (define/public (show-inner expr env inner result table)
      ...
      (match inner
        ...
        [(deriv:observe-sample dist value ?1 weight)
         ...
         (summary-exn ?1)
         (unless ?1
           (insert "density of " #:style meta-sd)
           (insert/abbrev (value->sexpr (node:eval-result value)) #hash() #:limit 30)
           (insert " in " #:style meta-sd)
           (insert/abbrev (value->sexpr (node:eval-result dist)) #hash() #:limit 40)
           (insert " = " #:style meta-sd)
           (insert/abbrev weight #hash())
           (insert "\n"))]
        [(deriv:fail ?1)
         (insert/rule "fail")
         (summary-exn ?1)]
